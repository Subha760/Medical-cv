...
export default function PdfPages({ blob, thumbnail=false }: { blob: Blob; thumbnail?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      } catch (cause) { console.error('PDF preview failed: '+(cause instanceof Error ? cause.name+': '+cause.message+' '+cause.stack : String(cause))); if(!cancelled) setError('Preview could not load. You can still download the PDF.'); }
    }
